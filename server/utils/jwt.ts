...
// Uses single JWT_SECRET for consistency across all authentication
...
if (!JWT_SECRET) {
  if (process.env.NODE_ENV === 'production') {
    ...
    console.warn(
      '⚠️  WARNING: JWT_SECRET not set. ' +
      'Using temporary secret for development. All tokens will be invalidated on server restart. ' +
      'Set JWT_SECRET environment variable for persistent authentication.'
    );
  }
}

// Use provided secret or generate temporary one for development
const FINAL_ACCESS_SECRET = JWT_SECRET || crypto.randomBytes(64).toString('hex');
const FINAL_REFRESH_SECRET = JWT_SECRET || crypto.randomBytes(64).toString('hex');
