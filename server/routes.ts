...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Setup session-based authentication with PostgreSQL store
  // This ensures sessions are shared across all server instances (Replit + stylemate.in)
  const sessionTtl = 30 * 24 * 60 * 60 * 1000; // 30 days
  
  // Cookie security: Allow override via COOKIE_SECURE env var for debugging proxy issues
  // Set COOKIE_SECURE=false on Hostinger to test if cookies are being rejected due to HTTPS proxy misconfiguration
  const cookieSecure = process.env.COOKIE_SECURE !== undefined 
    ? process.env.COOKIE_SECURE === 'true'
    : process.env.NODE_ENV === "production";
  
  console.log(`[Session Config] NODE_ENV=${process.env.NODE_ENV}, COOKIE_SECURE=${cookieSecure}`);
  ...
  // Create PostgreSQL session store for persistent, shared sessions
  ...
  app.post("/api/auth/register", async (req: Request, res: Response) => {
    try {
          ...
          if (decodedToken) {
            ...
            if (verifiedPhoneNumber && phone && verifiedPhoneNumber !== phone) {
              return res.status(400).json({
                error:
                  "Phone number mismatch. The verified phone number does not match the provided phone.",
              });
            }
      ...
      try {
        ...
        if (isBusinessUser) {
          const { sendBusinessWelcomeVerificationEmail } = await import(
            "./lib/sendgrid"
          );
          ...
          const { sendWelcomeVerificationEmail } = await import(
            "./lib/sendgrid"
          );
      ...
      res.status(200).json({
        ...
        user: userResponse,
        message:
          "Welcome to Stylemate! Your account has been created successfully.",
  ...
  app.post("/api/auth/resend-verification", async (req: any, res) => {
    try {
      ...
      if (userId) {
        ...
        return res.status(400).json({
          error:
            "Please provide your email address or log in to resend verification email.",
        });
      }
      ...
      const isBusinessUser =
        user.workPreference !== null || user.businessCategory !== null;
  ...
  app.post("/api/salons", isAuthenticated, async (req: any, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "Failed to create salon. Please try again." });
    }
  });
    ...
    try {
      ...
      if (!user.password) {
        return res.status(401).json({
          error:
            "This account was created with social authentication. Please contact support to set a password.",
        });
      }
      ...
      if (loginType === "customer" && isBusinessUser) {
        return res.status(403).json({
          error:
            "This is a business account. Please use the business login portal.",
      ...
      if (loginType === "business" && !isBusinessUser) {
        return res.status(403).json({
          error:
            "This is a customer account. Please use the customer login portal.",
    ...
    try {
      ...
      if (!firebaseToken || !phoneNumber || !newPassword) {
        return res
          ...
          .json({
            error:
              "Firebase token, phone number, and new password are required",
          });
      }
      ...
      const { verifyFirebaseToken, getPhoneNumberFromToken } = await import(
        "./firebaseAdmin"
      );
      ...
      try {
        ...
        if (!tokenPhone) {
          return res
            .status(400)
            .json({ error: "Firebase token does not contain a phone number" });
        }
  ...
  app.get("/api/salon-details/:salonId", async (req: any, res) => {
    try {
      ...
      const primaryImage =
        mediaAssets?.find(
          (asset: any) => asset.type === "image" && asset.isPrimary === 1,
        )?.url ||
  ...
  // PUBLIC: Get nearby salons based on current salon's location
  app.get("/api/salons/:salonId/nearby", async (req: any, res) => {
    try {
      const { salonId } = req.params;
      const limit = parseInt(req.query.limit as string) || 6;

      // Get the current salon to extract its coordinates
      ...
      // If salon doesn't have coordinates, return empty array
      ...
      // Query nearby salons using Haversine formula
      ...
      // Maximum distance: 25km radius (reasonable for "nearby" salons in urban areas)
      const maxDistanceKm = 3;

      const nearbySalons = await db
        .select({
          id: salons.id,
          name: salons.name,
          ...
          priceRange: salons.priceRange,
          rating: salons.rating,
          reviewCount: salons.reviewCount,
          googleRating: salons.googleRating,
          googleReviewCount: salons.googleReviewCount,
          imageUrl: salons.imageUrl,
          imageUrls: salons.imageUrls,
          openTime: salons.openTime,
          closeTime: salons.closeTime,
          distance: sql<number>`
            6371 * acos(
              cos(radians(${currentLat})) * 
              cos(radians(CAST(${salons.latitude} AS FLOAT))) * 
              cos(radians(CAST(${salons.longitude} AS FLOAT)) - radians(${currentLng})) + 
              sin(radians(${currentLat})) * 
              sin(radians(CAST(${salons.latitude} AS FLOAT)))
            )
          `.as("distance_km"),
        })
        .from(salons)
        .where(
          and(
            ...
            ne(salons.id, salonId),
            isNotNull(salons.latitude),
            isNotNull(salons.longitude),
      ...
      // Fetch media assets for each salon to get primary images
      const salonsWithImages = await Promise.all(
        nearbySalons.map(async (salon) => {
          ...
          const safeAssets = mediaAssets ?? []; // Ensure we have an array to work with

          // Get primary image URL (prefer primary gallery/cover/logo, fallback to any gallery/cover/logo)
          const imageTypes = ["gallery", "cover", "logo"];
          const primaryImageUrl =
            safeAssets.find(
              (asset: any) =>
                imageTypes.includes(asset.assetType) && asset.isPrimary === 1,
            )?.url ||
            safeAssets.find((asset: any) =>
              imageTypes.includes(asset.assetType),
            )?.url ||
            "";

          // Get all image URLs for gallery (first 4 images)
          const imageUrls = safeAssets
            .filter((asset: any) => imageTypes.includes(asset.assetType))
            .map((asset: any) => asset.url)
            .slice(0, 4);

          // Fallback to salon.imageUrls if no media assets and ensure it's always an array
          const finalImageUrls =
            imageUrls.length > 0
              ? imageUrls
              : Array.isArray(salon.imageUrls)
                ? salon.imageUrls
                : [];

          const hasGoogleReviews =
            salon.googleReviewCount && salon.googleReviewCount > 0;
    ...
    async (req: any, res) => {
      try {
        ...
        if (accessibleSalons.length <= 1) {
          return res.status(400).json({
            error:
              "Cannot delete your only salon. You must have at least one salon.",
          });
        }
      ...
      try {
        ...
        const setupStatus = {
          ...
          staff: {
            ...
            message:
              staff && staff.length > 0
                ? `${staff.length} team member(s) added`
                : "No staff added yet",
          },
          resources: {
          ...
          bookingSettings: {
            ...
            message: bookingSettings
              ? "Booking settings configured"
              : "Booking settings not configured",
          },
          paymentSetup: {
            ...
            message: payoutAccount
              ...
              : "Payment not configured - bookings will work but payments disabled",
          },
          media: {
            ...
            message:
              mediaAssets && mediaAssets.length > 0
                ? `${mediaAssets.length} photo(s) uploaded`
        ...
        if (!setupStatus.businessInfo.completed) {
          setupStatus.businessInfo.missingFields =
            setupStatus.businessInfo.requiredFields.filter(
        ...
        if (!setupStatus.locationContact.completed) {
          setupStatus.locationContact.missingFields =
            setupStatus.locationContact.requiredFields.filter(
              (field) => !salon[field as keyof typeof salon],
            );
        }
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        const { salonId } = req.params;
        const allCategories =
          await storage.getProductCategoriesBySalonId(salonId);
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        const { salonId } = req.params;
        const categories =
          await storage.createDefaultProductCategories(salonId);
        res
          .status(201)
          .json({
            success: true,
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        res
          .status(400)
          .json({ error: error.message || "Failed to create vendor" });
      }
    },
  );
  ...
  app.put(
    ...
    async (req: any, res) => {
      try {
        ...
        res
          .status(400)
          .json({ error: error.message || "Failed to update vendor" });
      }
    },
  );
  ...
  app.delete(
    ...
    async (req: any, res) => {
      try {
        ...
        res
          .status(400)
          .json({ error: error.message || "Failed to delete vendor" });
      }
    },
  );
    ...
    async (req: any, res) => {
      try {
        ...
        if (productIds.length > 0) {
          retailConfigs = await db
            .select()
            .from(schema.productRetailConfig)
        ...
        const products = productsData.map((product: any) => {
          const retailConfig = retailConfigs.find(
            (rc: any) => rc.productId === product.id,
          );
          return {
            ...product,
    ...
    async (req: any, res) => {
      try {
        ...
        const allocationSchema = z.object({
          retailStockAllocated: z
            .number()
            .min(0, "Stock allocation must be 0 or greater")
            .finite("Stock allocation must be a valid number"),
          retailPriceInPaisa: z
            .number()
            .int()
            .min(1, "Retail price must be at least ₹0.01")
            .optional(), // Customer-facing price
  ...
  app.get(
    ...
    async (req: any, res) => {
      try {
        ...
        res
          .status(500)
          .json({ error: "Failed to fetch product stock movements" });
      }
    },
  );
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        res
          .status(400)
          .json({ error: error.message || "Failed to create stock movement" });
      }
    },
  );
    ...
    async (req: any, res) => {
      try {
        ...
        if (!items || items.length === 0) {
          return res
            .status(400)
            .json({ error: "Purchase order must have at least one item" });
        }

        // Convert date string to Date object
        const deliveryDate = expected_delivery_date
        ...
        res
          .status(400)
          .json({ error: error.message || "Failed to create purchase order" });
      }
    },
  );
  ...
  app.put(
    ...
    async (req: any, res) => {
      try {
        ...
        res
          .status(400)
          .json({ error: error.message || "Failed to approve purchase order" });
      }
    },
  );
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        res
          .status(400)
          .json({ error: error.message || "Failed to receive purchase order" });
      }
    },
  );
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        res
          .status(400)
          .json({ error: error.message || "Failed to confirm purchase order" });
      }
    },
  );
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        res
          .status(400)
          .json({ error: error.message || "Failed to mark as delivered" });
      }
    },
  );
  ...
  app.delete(
    ...
    async (req: any, res) => {
      try {
        ...
        res
          .status(400)
          .json({ error: error.message || "Failed to delete purchase order" });
      }
    },
  );
  ...
  app.post("/api/auth/check-profile-completion-disabled", async (req, res) => {
    try {
      ...
      const readinessCheck =
        await storage.checkBusinessReadiness(primarySalonId);

      // Determine next step based on missing requirements
      let nextStep = "review-publish";
      if (
        readinessCheck.missingRequirements.includes(
          "Complete salon basic information",
        )
      ) {
        nextStep = "business-info";
      } else if (
        readinessCheck.missingRequirements.includes("Add at least one service")
      ) {
        nextStep = "services";
      } else if (
        readinessCheck.missingRequirements.includes(
        ...
        readinessCheck.missingRequirements.includes("Configure payout account")
      ) {
      ...
      res
        .status(500)
        .json({ error: "Failed to check profile completion status" });
    }
  });
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        const savedLocations =
          await storage.getUserSavedLocationsByUserId(userId);
  ...
  app.post(
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        const validation =
          insertUserSavedLocationSchema.safeParse(locationData);
        ...
        const newLocation =
          await storage.createUserSavedLocation(validatedData);
  ...
  app.put(
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        const partialSchema = insertUserSavedLocationSchema
          .omit({ userId: true })
          .partial();
    ...
    try {
      const updates = [
        ...
        },
        {
          ...
          location:
            "Nirala Estate, Tech Zone IV, Greater Noida West, Uttar Pradesh",
          address:
            "Shop no 102, Nirala Estate, Tech Zone IV, Greater Noida West",
          latitude: "28.5360",
          longitude: "77.3920",
        },
        {
      ...
      for (const salonId of salonIds) {
        ...
        for (const pattern of patterns) {
          ...
          try {
            const createdPattern =
              await storage.createAvailabilityPattern(salonPattern);
        ...
        for (const staffMember of staff) {
          for (const pattern of patterns) {
            ...
            try {
              const createdPattern =
                await storage.createAvailabilityPattern(staffPattern);
              createdPatterns.push(createdPattern);
    ...
    async (req, res) => {
      ...
      try {
        ...
        if (redisClient) {
          ...
          if (
            inMemoryCached &&
            Date.now() - inMemoryCached.timestamp < CACHE_DURATION * 1000
          ) {
        ...
        const searchResults = delhiNCRLocations.filter((loc) => {
          ...
          const fullText =
            `${loc.name} ${loc.area} ${loc.state} ${loc.country}`.toLowerCase();
        ...
        try {
          ...
          if (googleResponse.ok) {
            ...
            if (data.status === "OK" && data.predictions) {
              ...
              const placeDetailsPromises = data.predictions
                .slice(0, 8)
                .map(async (prediction: any) => {
                  try {
                    ...
                    const detailsParams = new URLSearchParams({
                      ...
                      key: googlePlacesKey,
                      fields:
                        "formatted_address,geometry,name,address_components",
                    });
              ...
              if (redisClient) {
                try {
                  ...
                  console.log(
                    `💾 Cached Google Places in Redis: "${validatedParams.q}"`,
                  );
                } catch (error) {
                  console.error("Redis cache write error:", error);
                  locationSearchCache.set(cacheKey, cacheData);
                ...
                locationSearchCache.set(cacheKey, cacheData);
                console.log(
                  `💾 Cached Google Places in memory: "${validatedParams.q}"`,
                );
              }
            } else {
              console.log(
          ...
          // Smart search algorithm for Delhi NCR with fuzzy matching
          const searchResults = delhiNCRLocations.filter((loc) => {
            const searchTerms = fallbackQuery
              .split(" ")
              .filter((term) => term.length > 0);
            const fullText =
              `${loc.name} ${loc.area} ${loc.state} ${loc.country}`.toLowerCase();
        ...
        const findNearest = (locations: typeof delhiNCRLocations) => {
          ...
          for (const loc of locations) {
            const distance = Math.sqrt(
              Math.pow(lat - loc.coords.lat, 2) +
                Math.pow(lng - loc.coords.lng, 2),
            );
    ...
    async (req, res) => {
      try {
        ...
        if (
          ...
          geoapifyResponse.features.length === 0
        ) {
          return res.status(404).json({
                ...
                results: [
                  {
                    ...
                    formatted_address: feature.properties.formatted,
                    address_components:
                      feature.properties.address_components || [],
                  },
                ],
              };
            }
  ...
  const makeGeoapifyRequest = async (
    url: string,
  ...
  const makeNominatimRequest = async (
    url: string,
    params: Record<string, string>,
    options: RequestInit = {},
  ): Promise<any> => {
      ...
      try {
        ...
        if (GEOAPIFY_API_KEY) {
          try {
            ...
            suggestions =
              geoapifyResponse.features?.map((feature: any) => ({
                id:
                  feature.properties.place_id ||
                  `${feature.geometry.coordinates[1]}_${feature.geometry.coordinates[0]}`,
                title:
                  feature.properties.formatted || feature.properties.name || "",
                subtitle: [
                  feature.properties.city,
        ...
        res.json({
          ...
          source:
            GEOAPIFY_API_KEY && suggestions.length > 0
              ? "geoapify"
              : "nominatim",
        });
      } catch (error) {
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const isReverseGeocoding =
          "lat" in validatedData && "lng" in validatedData;
        const isForwardGeocoding = "address" in validatedData;

        if (isReverseGeocoding) {
          // Reverse geocoding: coordinates -> address
          const { lat, lng, countrycode } = validatedData as {
            lat: number;
            lng: number;
            countrycode?: string;
          };
          ...
          if (!result) {
            try {
              ...
              if (nominatimResponse) {
                const address =
                  nominatimResponse.display_name || `${lat}, ${lng}`;
                result = {
                  address: address,
                  lat: lat,
                  lng: lng,
          ...
          if (!result) {
            return res.status(404).json({
              error: "Location not found",
              message:
                "The specified coordinates could not be reverse geocoded",
            });
          }
        } else if (isForwardGeocoding) {
          // Forward geocoding: address -> coordinates
          const { address, countrycode } = validatedData as {
            address: string;
            countrycode?: string;
          };
    ...
    try {
      ...
      if (booking && booking.offerId) {
        ...
        try {
          ...
          const user = booking.guestSessionId
            ? null
            : await storage.getUserByEmail(booking.customerEmail);
          ...
          if (user) {
            ...
            if (!eligibility.eligible) {
              console.error(
                `🚨 CRITICAL: Offer ${booking.offerId} usage limit exceeded for user ${user.id}. Reason: ${eligibility.reason}`,
              );
              console.error(
  ...
  app.get("/api/search/salons", spikeProtection, async (req, res) => {
    try {
      ...
      console.log(
        `🌍 Searching salons near (${params.lat}, ${params.lng}) within ${params.radiusKm}km`,
      );
        ...
        if (matchingCategories.size > 0) {
          ...
          filteredResults = filteredResults.filter(
            (salon) =>
              ...
              (salon.description &&
                salon.description.toLowerCase().includes(searchQuery)) ||
      ...
      if (rawParams.instantBooking) {
        filteredResults = filteredResults.filter(
          (salon) => (salon as any).instantBooking === 1,
        );
      }
      ...
      if (rawParams.offerDeals) {
        filteredResults = filteredResults.filter(
          (salon) => (salon as any).offerDeals === 1,
        );
      }
      ...
      if (rawParams.acceptGroup) {
        filteredResults = filteredResults.filter(
          (salon) => (salon as any).acceptGroup === 1,
        );
      }
          ...
          filteredResults.map(async (salon) => {
            try {
              ...
              const hasAvailability = timeSlots.some((slot: any) => {
                ...
                const slotStartHour =
                  slotStart.getHours() + slotStart.getMinutes() / 60;
                const slotEndHour =
        ...
        case "recommended":
          ...
          filteredResults.sort((a, b) => {
            const scoreA =
              ...
              Math.log10((a.reviewCount || 1) + 1);
            const scoreB =
              parseFloat(b.rating || "0") *
              Math.log10((b.reviewCount || 1) + 1);
            ...
            try {
              ...
              if (response.ok) {
                ...
                if (data.routes && data.routes.length > 0) {
                  ...
                  console.log(
                    `🚗 ${salon.name}: Straight-line ${salon.distance.toFixed(1)}km → Driving ${drivingDistanceKm.toFixed(1)}km`,
                  );
                }
              }
            } catch (error) {
      ...
      const response: SalonSearchResult = {
        salons: await Promise.all(
          salonsWithDrivingDistance.map(async (salon) => {
            ...
            const mediaAssets = await storage.getMediaAssetsBySalonId(salon.id);
            const primaryImage =
              mediaAssets?.find((asset: any) => asset.isPrimary)?.url ||
              mediaAssets?.find((asset: any) => asset.assetType === "cover")
                ?.url ||
              "";
            ...
            try {
              ...
              if (params.date) {
                if (params.date === "tomorrow") {
                  searchDate = new Date(Date.now() + 24 * 60 * 60 * 1000)
                    .toISOString()
                    .split("T")[0];
                } else if (params.date === "today") {
              ...
              if (timeSlots.length === 0 && salon.businessHours) {
                ...
                if (
                  ...
                  const [startHour, startMin] = dayHours.start
                    .split(":")
                    .map(Number);
              ...
              const isSlotBooked = (slotTime: Date): boolean => {
                ...
                for (const booking of bookings) {
                  ...
                  const bookingEndMinutes =
                    bookingStartMinutes + bookingDuration;
              ...
              if (params.time) {
                ...
                processedSlots = processedSlots.filter((slot: any) => {
                  ...
                  const slotHour =
                    slotDateTime.getHours() + slotDateTime.getMinutes() / 60;
                  return slotHour >= startHour && slotHour < endHour;
                });
              }

              // Return only available slots first, then show up to 3 booked/past for reference
              const availableSlots = processedSlots
                .filter((s: any) => s.available)
                .slice(0, 6);
              const unavailableSlots = processedSlots
                .filter((s: any) => !s.available)
                .slice(0, 3);
      ...
      if (error instanceof Error) {
        if (
          error.message.includes("Latitude must be") ||
  ...
  app.get("/api/salons", async (req, res) => {
    try {
      ...
      let formattedSalons = await Promise.all(
        salonsWithDistance.map(async (salon) => {
          try {
            // Get media assets for this salon
            const mediaAssets = await storage.getMediaAssetsBySalonId(salon.id);
            const safeAssets = mediaAssets ?? []; // Ensure we have an array to work with
            ...
            const primaryImageUrl =
              ...
              safeAssets.find((asset) => imageTypes.includes(asset.assetType))
                ?.url ||
              "";
            ...
            // Get all image URLs for gallery (first 4 images - includes gallery, cover, logo)
            const imageUrls = safeAssets
              .filter((asset) => imageTypes.includes(asset.assetType))
              .map((asset) => asset.url)
              .slice(0, 4);

            console.log(
              `Salon ${salon.name}: Found ${safeAssets.length} media assets, ${imageUrls.length} images for gallery, ${services.length} services, ${packages.length} packages, ${reviews.length} Google reviews, primary: ${primaryImageUrl || "none"}`,
            );
            ...
            try {
              if (
                typeof salon.category === "string" &&
                salon.category.startsWith("[")
              ) {
                const categories = JSON.parse(salon.category);
                categoryDisplay = Array.isArray(categories)
                  ? categories[0] || "Beauty Services"
                  : salon.category;
              }
            } catch (e) {
            ...
            return {
              ...
              image: primaryImageUrl || "", // Include primary image URL
              imageUrls, // Add image gallery array for SalonCard
              latitude: salon.latitude,
              longitude: salon.longitude,
              services: serviceDetails, // Add services array
              distance_km: salon.distance
                ? Math.max(0.01, Number(salon.distance.toFixed(2)))
                : undefined, // ACTUAL driving distance
            ...
            return {
              ...
              imageUrls: [], // Empty image gallery on error
              latitude: salon.latitude,
              longitude: salon.longitude,
      ...
      if (categories && typeof categories === "string") {
        ...
        formattedSalons = formattedSalons.filter((salon) =>
          categoryList.some((cat) =>
            salon.category.toLowerCase().includes(cat.toLowerCase()),
          ),
        );
      }
      ...
      try {
        ...
        for (const bookingId of input.bookingIds) {
          ...
          if (!booking) {
            return res
              .status(404)
              .json({ error: `Booking ${bookingId} not found` });
          }
    ...
    async (req: any, res) => {
      try {
        ...
        try {
          ...
          const staff = updatedBooking.staffId
            ? await storage.getStaff(updatedBooking.staffId)
            : null;
        ...
        if (error instanceof Error) {
          if (
            ...
            error.message.includes("conflicts with existing booking")
          ) {
            return res.status(409).json({
  ...
  app.get(
    ...
    async (req: any, res) => {
      try {
        ...
        res
          .status(500)
          .json({ error: "Failed to fetch customer segmentation" });
      }
    },
  );
  ...
  app.get("/api/salons/:salonId/services", async (req: any, res) => {
    try {
      ...
      const t1 = Date.now();
      console.log(
        `[perf] getServicesBySalonId(salonId=${salonId}) took ${t1 - t0}ms, count=${services?.length ?? 0}`,
      );
      res.json(services || []);
    } catch (error) {
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        const { serviceIds, name, description, discountedPricePaisa } =
          validationResult.data;
        ...
        if (!serviceIds || serviceIds.length < 2) {
          return res
            .status(400)
            .json({ error: "Package must include at least 2 services" });
        }
        ...
        if (invalidService === undefined && services.some((s) => !s)) {
          return res
            .status(400)
            .json({ error: "One or more services not found" });
        }
        if (invalidService) {
          return res
            .status(400)
            .json({ error: "All services must belong to this salon" });
        }
        ...
        const regularPricePaisa = services.reduce(
          (sum, s) => sum + (s?.priceInPaisa || 0),
          0,
        );
        ...
        if (regularPricePaisa <= 0) {
          return res
            .status(400)
            .json({ error: "Services must have a total price greater than 0" });
        }
        ...
        const discountPercentage = Math.round(
          ((regularPricePaisa - discountedPricePaisa) / regularPricePaisa) *
            100,
        );
      ...
      try {
        ...
        if (serviceIds && serviceIds.length > 0) {
          ...
          const finalDiscountedPrice =
            discountedPricePaisa || existingPackage.packagePriceInPaisa;
          ...
          const discountPercentage = Math.round(
            ((regularPricePaisa - finalDiscountedPrice) / regularPricePaisa) *
              100,
          );
            ...
            packageId,
            {
              ...
              ...(discountedPricePaisa && {
                ...
                discountPercentage: Math.round(
                  ((existingPackage.regularPriceInPaisa -
                    discountedPricePaisa) /
                    existingPackage.regularPriceInPaisa) *
                    100,
                ),
    ...
    try {
      ...
      if (
        ...
        return res
          ...
          .json({
            error:
              "Missing required fields: salonId and serviceIds are required",
          });
      }
      ...
      for (let i = 0; i < services.length; i++) {
        ...
        if (!service || !service.isActive) {
          return res
            .status(404)
            .json({ error: `Service ${serviceIds[i]} not found or inactive` });
        }
      ...
      if (staffId) {
        ...
        if (staff.salonId !== salonId) {
          return res
            .status(400)
            .json({ error: "Staff member does not belong to this salon" });
        }
      }
      ...
      if (offerId) {
        ...
        if (!offer.isPlatformWide && offer.salonId !== salonId) {
          return res
            .status(400)
            .json({ error: "Offer does not apply to this salon" });
        }
  ...
  app.post("/api/create-payment-order", async (req, res) => {
    try {
      ...
      if (service.salonId !== input.salonId) {
        ...
        return res
          .status(400)
          .json({ error: "Service does not belong to the specified salon" });
      }
      ...
      try {
        ...
        return res
          .status(500)
          .json({ error: "Failed to validate booking time slot" });
      }
      ...
      const booking = await storage.createBooking({
        ...
        status:
          input.booking.paymentMethod === "pay_at_salon"
            ? "confirmed"
            : "pending",
        ...
        originalAmountPaisa: input.booking.offerId
          ? service.priceInPaisa
          : null,
        discountAmountPaisa: input.booking.offerId
          ? offerValidation.discountAmount
          : null,
        finalAmountPaisa: input.booking.offerId
          ? offerValidation.finalAmount
          : null,
      });
      ...
      if (input.booking.paymentMethod === "pay_at_salon") {
        ...
        res.json({
          ...
          original_amount: input.booking.offerId
            ? service.priceInPaisa
            : undefined,
          discount: input.booking.offerId
            ? offerValidation.discountAmount
            : undefined,
          currency: service.currency,
          status: "confirmed",
  ...
  app.post("/api/verify-payment", async (req, res) => {
    try {
      ...
      try {
        ...
        return res
          .status(400)
          .json({ error: "Failed to verify payment with Razorpay" });
      }
  ...
  app.get(
    ...
    async (req: any, res) => {
      try {
        ...
        const { getSuggestedRolesFromServices } = await import(
          "../shared/service-role-mapping.js"
        );
  ...
  app.get(
    ...
    async (req: any, res) => {
      try {
        const { salonId } = req.params;
        const patterns =
          await storage.getAvailabilityPatternsBySalonId(salonId);
        res.json(patterns);
      } catch (error) {
        console.error("Error fetching availability patterns:", error);
        res
          .status(500)
          .json({ error: "Failed to fetch availability patterns" });
      }
    },
  );
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        res
          .status(500)
          .json({ error: "Failed to create availability pattern" });
      }
    },
  );
    ...
    async (req: any, res) => {
      try {
        ...
        if (!pattern) {
          return res
            .status(404)
            .json({ error: "Availability pattern not found" });
        }
        ...
        res
          .status(500)
          .json({ error: "Failed to update availability pattern" });
      }
    },
  );
  ...
  app.delete(
    ...
    async (req: any, res) => {
      try {
        ...
        res
          .status(500)
          .json({ error: "Failed to delete availability pattern" });
      }
    },
  );
    ...
    async (req: any, res) => {
      try {
        ...
        if (!hasManagementAccess && !isStaff) {
          return res
            .status(403)
            .json({ error: "Access denied to manage this time slot" });
        }
  ...
  app.put(
    ...
    async (req: any, res) => {
      try {
        ...
        // Validate input using Zod schema (partial updates allowed)
        const partialSchema = insertBookingSettingsSchema
          .omit({ salonId: true })
          .partial();
        const validationResult = partialSchema.safeParse(req.body);
        if (!validationResult.success) {
  ...
  app.put(
    ...
    async (req: any, res) => {
      try {
        ...
        // Validate input using Zod schema (partial updates allowed)
        const partialSchema = insertResourceSchema
          .omit({ salonId: true })
          .partial();
        const validationResult = partialSchema.safeParse(req.body);
        if (!validationResult.success) {
        ...
        if (existingResource.salonId !== salonId) {
          return res
            .status(403)
            .json({ error: "Resource does not belong to this salon" });
        }
    ...
    async (req: any, res) => {
      try {
        ...
        if (staffId && serviceId) {
          ...
          const staffServices =
            await storage.getStaffServicesBySalonId(salonId);
      ...
      try {
        ...
        if (serviceId && resourceId) {
          ...
          const serviceResources =
            await storage.getServiceResourcesBySalonId(salonId);
    ...
    async (req: any, res) => {
      try {
        ...
        if (!salonId || !mediaId) {
          return res
            .status(400)
            .json({ error: "Salon ID and Media ID are required" });
        }
        ...
        if (existingAsset.salonId !== salonId) {
          return res
            .status(403)
            .json({ error: "Media asset does not belong to this salon" });
        }
  ...
  app.put(
    ...
    async (req: any, res) => {
      try {
        ...
        // Validate input using Zod schema (partial updates allowed)
        const partialSchema = insertTaxRateSchema
          .omit({ salonId: true })
          .partial();
        const validationResult = partialSchema.safeParse(req.body);
        if (!validationResult.success) {
        ...
        if (existingTaxRate.salonId !== salonId) {
          return res
            .status(403)
            .json({ error: "Tax rate does not belong to this salon" });
        }
  ...
  app.put(
    ...
    async (req: any, res) => {
      try {
        ...
        // Validate input using Zod schema (partial updates allowed)
        const partialSchema = insertPublishStateSchema
          .omit({ salonId: true })
          .partial();
        const validationResult = partialSchema.safeParse(req.body);
        if (!validationResult.success) {
  ...
  app.get(
    ...
    async (req: any, res) => {
      try {
        ...
        res
          .status(500)
          .json({ error: "Failed to fetch dashboard completion status" });
      }
    },
  );
  ...
  app.put(
    ...
    async (req: any, res) => {
      try {
        const { salonId, categoryId } = req.params;
        const partialSchema = insertExpenseCategorySchema
          .omit({ salonId: true })
          .partial();
        const validationResult = partialSchema.safeParse(req.body);
        if (!validationResult.success) {
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        const { salonId } = req.params;
        const categories =
          await storage.createDefaultExpenseCategories(salonId);
        res.status(201).json(categories);
      } catch (error) {
        console.error("Error creating default expense categories:", error);
        res
          .status(500)
          .json({ error: "Failed to create default expense categories" });
      }
    },
  );
  ...
  app.put(
    ...
    async (req: any, res) => {
      try {
        ...
        const partialSchema = insertExpenseSchema
          .omit({ salonId: true, createdBy: true })
          .partial();
        const validationResult = partialSchema.safeParse(req.body);
        if (!validationResult.success) {
          return res.status(400).json({
  ...
  app.put(
    ...
    async (req: any, res) => {
      try {
        ...
        const partialSchema = insertCommissionRateSchema
          .omit({ salonId: true })
          .partial();
        const validationResult = partialSchema.safeParse(req.body);
        if (!validationResult.success) {
  ...
  app.get(
    ...
    async (req: any, res) => {
      try {
        const { salonId } = req.params;
        const { staffId, period, paymentStatus, startDate, endDate } =
          req.query;
        const filters = {
        ...
        const commissions = await storage.getCommissionsBySalonId(
          salonId,
          filters,
        );
  ...
  app.put(
    ...
    async (req: any, res) => {
      try {
        ...
        const partialSchema = insertBudgetSchema
          .omit({ salonId: true, createdBy: true })
          .partial();
        const validationResult = partialSchema.safeParse(req.body);
        if (!validationResult.success) {
  ...
  app.get(
    ...
    async (req: any, res) => {
      try {
        ...
        res
          .status(500)
          .json({ error: "Failed to generate cash flow statement" });
      }
    },
  );
  ...
  */
    ...
    async (req: any, res) => {
      try {
        ...
        if (existingCampaign.salonId !== salonId) {
          return res
            .status(403)
            .json({ error: "Access denied to this A/B test campaign" });
        }
    ...
    async (req: any, res) => {
      try {
        ...
        if (variant.testCampaignId !== testId) {
          return res
            .status(404)
            .json({ error: "Test variant not found in this campaign" });
        }
      ...
      try {
        ...
        if (validationResult.data.templateOverrides) {
          try {
            ...
            return res
              .status(400)
              .json({ error: "Invalid template overrides JSON" });
          }
        }
      ...
      try {
        ...
        if (validationResult.data.variantId) {
          ...
          if (!variant || variant.testCampaignId !== testId) {
            return res
              .status(400)
              .json({ error: "Invalid variant ID for this test campaign" });
          }
        }
    ...
    async (req: any, res) => {
      try {
        ...
        if (!winnerVariantId) {
          return res
            .status(400)
            .json({ error: "Winner variant ID is required" });
        }
      ...
      try {
        ...
        if (variants.length === 0) {
          return res
            ...
            .json({
              error:
                "Campaign must have at least one test variant before starting",
            });
        }
    ...
    async (req: any, res) => {
      try {
        ...
        if (campaign.status === "completed") {
          return res
            .status(400)
            .json({ error: "Campaign is already completed" });
        }
    ...
    async (req: any, res) => {
      try {
        ...
        const performanceSummary =
          await storage.getAbTestPerformanceSummary(testId);
        ...
        if (
          ...
          performanceSummary.variants.length > 1
        ) {
          // Find best performing variant based on success metric
          const sortedVariants = performanceSummary.variants.sort((a, b) => {
            const metricA =
              campaign.successMetric === "open_rate"
                ? a.openRate
                : campaign.successMetric === "click_rate"
                  ? a.clickRate
                  : campaign.successMetric === "conversion_rate"
                    ? a.conversionRate
                    : a.bookingRate;
            const metricB =
              campaign.successMetric === "open_rate"
          ...
          if (
            controlVariant &&
            bestVariant &&
            performanceSummary.totalParticipants > 100
          ) {
            const controlMetric =
              campaign.successMetric === "open_rate"
                ? controlVariant.openRate
                : campaign.successMetric === "click_rate"
                  ? controlVariant.clickRate
                  : campaign.successMetric === "conversion_rate"
                    ? controlVariant.conversionRate
                    : controlVariant.bookingRate;
            const bestMetric =
              campaign.successMetric === "open_rate"
                ? bestVariant.openRate
                : campaign.successMetric === "click_rate"
        ...
        res
          .status(500)
          .json({ error: "Failed to check auto-optimization status" });
      }
    },
  );
      ...
      try {
        ...
        if (!["cancelled", "confirmed", "pending"].includes(status)) {
          return res
            ...
            .json({
              error:
                "Invalid status. Must be one of: cancelled, confirmed, pending",
            });
        }
        ...
        if (error.message === "Booking not found or access denied") {
          res
            .status(404)
            .json({ error: "Appointment not found or access denied" });
        } else {
      ...
      try {
        ...
        for (const salon of activeSalons) {
          const salonDetail = {
            ...
            originalLat: salon.latitude
              ? parseFloat(salon.latitude as string)
              : null,
            originalLng: salon.longitude
              ? parseFloat(salon.longitude as string)
              : null,
              ...
              if (geocodeData.lat && geocodeData.lng) {
                ...
                if (
                  ...
                  const coordsChanged =
                    !salon.latitude ||
                    !salon.longitude ||
                    Math.abs(newLat - parseFloat(salon.latitude as string)) >
                      0.001 ||
                    Math.abs(newLng - parseFloat(salon.longitude as string)) >
                      0.001;
    ...
    async (req: any, res) => {
      try {
        ...
        if (!offerAfter) {
          return res
            .status(500)
            .json({ error: "Failed to retrieve updated offer" });
        }
        ...
        res.json({
          ...
          _meta: {
            requiresReapproval:
              offerBefore.approvalStatus === "approved" &&
              offerAfter.approvalStatus === "pending",
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        res
          .status(500)
          .json({ error: error.message || "Failed to import Google reviews" });
      }
    },
  );
  ...
  app.get("/api/salons/:salonId/reviews", async (req, res) => {
    try {
      ...
      const sourceFilter =
        source === "google" || source === "stylemate" ? source : undefined;
      ...
      try {
        ...
        const offers = await storage.getAllOffers({
          ...
          isPlatformWide: isPlatformWide
            ? parseInt(isPlatformWide as string)
            : undefined,
          salonId: salonId as string,
        ...
        if (autoApproved !== undefined) {
          const isAutoApproved =
            autoApproved === "1" || autoApproved === "true";
  ...
  app.post("/api/offers/calculate", async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const activeOffers = offers.filter(
        (offer: any) =>
          offer.isActive === 1 && offer.approvalStatus === "approved",
      );
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        if (!salonId || !promoCode || totalAmountPaisa === undefined) {
          return res.status(400).json({
            error:
              "Missing required fields: salonId, promoCode, and totalAmountPaisa",
          });
        }
  ...
  app.get("/api/beauty-products", async (req, res) => {
    try {
      ...
      if (skinTone) {
        ...
        conditions.push(
          sql`${schema.beautyProducts.skinToneCompatibility} ILIKE ${skinTonePattern}`,
        );
      }
      ...
      const DEFAULT_PLACEHOLDER_IMAGE =
        "https://placehold.co/400x400/f3f4f6/94a3b8?text=No+Image";
  ...
  console.log('✅ Staff Payroll routes registered');
  ...
  app.get("/api/products/search", async (req, res) => {
    try {
      ...
      if (!query || query === "") {
        const rawProducts = await storage.getRetailProducts(
          (salonId as string) || "",
          {
            categoryId: categoryId as string,
            minPrice: minPrice ? parseInt(minPrice as string) : undefined,
        ...
        // Fetch salon names for all unique salon IDs (batched with Promise.all)
        const uniqueSalonIds = [...new Set(filteredProducts.map((p: any) => p.salonId).filter(Boolean))] as string[];
        const salonResults = await Promise.all(
          uniqueSalonIds.map((sid) => storage.getSalonById(sid))
        );
        const salonNameMap: Record<string, string> = {};
        salonResults.forEach((salon, index) => {
          if (salon) {
            salonNameMap[uniqueSalonIds[index]] = salon.name;
          }
        });
        ...
        const products = filteredProducts.map((p: any) => ({
            ...
            salonId: p.salonId,
            salonName: salonNameMap[p.salonId] || null,
          }));
      ...
      //    - If useAllocatedStock=1: retailStockAllocated > 0
      //    - If useAllocatedStock=0: currentStock > 0
      const filteredSearchProducts = rawProducts
        .filter((p: any) => {
          const effectiveStock =
            p.retailConfig?.useAllocatedStock === 0
              ? parseFloat(String(p.currentStock || 0))
      ...
      // Fetch salon names for all unique salon IDs (batched with Promise.all)
      const uniqueSearchSalonIds = [...new Set(filteredSearchProducts.map((p: any) => p.salonId).filter(Boolean))] as string[];
      const searchSalonResults = await Promise.all(
        uniqueSearchSalonIds.map((sid) => storage.getSalonById(sid))
      );
      const searchSalonNameMap: Record<string, string> = {};
      searchSalonResults.forEach((salon, index) => {
        if (salon) {
          searchSalonNameMap[uniqueSearchSalonIds[index]] = salon.name;
        }
      });

      const products = filteredSearchProducts.map((p: any) => ({
          id: p.id,
          name: p.name,
          brand: p.brand,
          retailPriceInPaisa: p.retailPriceInPaisa,
          ...
          reviewCount: 0, // TODO: Implement reviews
          salonId: p.salonId,
          salonName: searchSalonNameMap[p.salonId] || null,
        }));
  ...
  app.get("/api/products/:productId", async (req, res) => {
    try {
      ...
      const product = {
        ...
        // Hybrid stock mode: Use warehouse stock if useAllocatedStock=0, else use allocated stock
        stock:
          rawProduct.retailConfig?.useAllocatedStock === 0
      ...
      try {
        ...
        if (!salonId) {
          ...
          const transformedItems = items.map((item) => {
            ...
            const retailPriceInPaisa = item.product?.retailConfig
              ?.retailPriceInPaisa
              ? parseInt(
            ...
            const productImage =
              retailImages.length > 0 ? retailImages[0] : null;

            // Determine stock based on use_allocated_stock flag
            const useAllocatedStock =
              item.product?.retailConfig?.useAllocatedStock || 0;
            const stock =
              useAllocatedStock === 1
          ...
          const subtotalInPaisa = transformedItems.reduce(
            (sum, item) => sum + item.totalPriceInPaisa,
            0,
          );
          ...
          const totalInPaisa =
            subtotalInPaisa + taxInPaisa + deliveryChargeInPaisa;
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        if (!product.availableForRetail) {
          return res
            .status(400)
            .json({ error: "Product not available for retail" });
        }
        ...
        if (variantId && product.variants) {
          ...
          if (variant.priceAdjustmentPaisa) {
            serverPrice =
              product.retailPriceInPaisa + variant.priceAdjustmentPaisa;
          }
        }
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        if (invalidItems.length > 0) {
          return res.status(400).json({
            error:
              "Cannot place order with items from multiple salons. Please checkout one salon at a time.",
          });
        }
        ...
        if (fulfillmentType === "delivery") {
          ...
          if (settings?.enableHomeDelivery === 1) {
            ...
            if (
              settings.freeDeliveryAbovePaisa &&
              subtotalPaisa >= settings.freeDeliveryAbovePaisa
            ) {
              deliveryFeePaisa = 0;
            }
          } else {
            return res
              .status(400)
              .json({ error: "Home delivery not enabled for this salon" });
          }
        }
        ...
        try {
          ...
          if (paymentMethod === "online" || paymentMethod === "upi") {
            try {
              ...
              const razorpayOrder =
                await razorpay.orders.create(razorpayOrderOptions);
        ...
        try {
          ...
          if (order && validation.data.status) {
            ...
            if (user && salon && user.email) {
              ...
              if (
                ["processing", "packed", "shipped", "delivered"].includes(
                  newStatus,
                )
              ) {
                await sendOrderStatusUpdate(
                  ...
                  newStatus,
                  {
                    ...
                    estimated_delivery:
                      validation.data.estimatedDeliveryDate || "TBD",
                    delivered_date:
                      newStatus === "delivered"
                        ? new Date().toLocaleDateString("en-IN")
                        : "",
                    delivery_method_pickup:
                      order.fulfillmentType === "pickup" ? "true" : "",
          ...
          if (order) {
            ...
            if (user && salon && user.email) {
              ...
              await sendOrderCancellation(salonId, orderId, user.email, {
                ...
                refund_applicable:
                  refundAmountPaisa && refundAmountPaisa > 0 ? "true" : "",
                refund_amount: refundAmountPaisa
                ...
                refund_timeline:
                  order.paymentMethod === "cod" ? "N/A" : "5-7 business days",
              });
  ...
  app.put(
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        // Validate request body with Zod
        const validation = schema.updateDeliverySettingsSchema.safeParse(
          req.body,
        );
        if (!validation.success) {
          return res.status(400).json({
