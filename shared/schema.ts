...
// Staff Employment Profiles - employment terms, compensation model, onboarding status
export const staffEmploymentProfiles = pgTable("staff_employment_profiles", {
  ...
  compensationModel: varchar("compensation_model", { length: 30 }).notNull().default('commission_only'),
  ...
  check("employment_type_valid", sql`employment_type IN ('full_time', 'part_time', 'contract', 'freelancer')`),
  check("compensation_model_valid", sql`compensation_model IN ('fixed_salary', 'hourly', 'commission_only', 'salary_plus_commission')`),
  check("onboarding_status_valid", sql`onboarding_status IN ('pending', 'in_progress', 'completed', 'on_hold')`),
  check("employment_status_valid", sql`status IN ('active', 'on_leave', 'notice_period', 'terminated', 'resigned')`),
  check("payout_method_valid", sql`preferred_payout_method IN ('bank_transfer', 'upi', 'cash')`),
]);

export const insertStaffEmploymentProfileSchema = createInsertSchema(staffEmploymentProfiles).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export type StaffEmploymentProfile = typeof staffEmploymentProfiles.$inferSelect;
export type InsertStaffEmploymentProfile = z.infer<typeof insertStaffEmploymentProfileSchema>;

// Staff Salary Components - versioned salary structure with effective dating
export const staffSalaryComponents = pgTable("staff_salary_components", {
  ...
  hourlyRatePaisa: integer("hourly_rate_paisa").default(0),
  dailyRatePaisa: integer("daily_rate_paisa").default(0),
  ...
  payoutDayOfMonth: integer("payout_day_of_month").default(1),
  ...
  index("salary_components_salon_idx").on(table.salonId),
  index("salary_components_effective_idx").on(table.effectiveFrom, table.effectiveTo),
  check("payout_frequency_valid", sql`payout_frequency IN ('weekly', 'bi_weekly', 'monthly')`),
  check("is_active_valid_salary", sql`is_active IN (0, 1)`),
]);

export const insertStaffSalaryComponentSchema = createInsertSchema(staffSalaryComponents).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export type StaffSalaryComponent = typeof staffSalaryComponents.$inferSelect;
export type InsertStaffSalaryComponent = z.infer<typeof insertStaffSalaryComponentSchema>;

// Staff Leave Types - salon-specific leave policy definitions
export const staffLeaveTypes = pgTable("staff_leave_types", {
  ...
  accrualRate: decimal("accrual_rate", { precision: 5, scale: 2 }).default('1.00'),
  ...
  check("accrual_type_valid", sql`accrual_type IN ('annual', 'monthly', 'quarterly')`),
  check("is_paid_valid", sql`is_paid IN (0, 1)`),
]);

export const insertStaffLeaveTypeSchema = createInsertSchema(staffLeaveTypes).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export type StaffLeaveType = typeof staffLeaveTypes.$inferSelect;
export type InsertStaffLeaveType = z.infer<typeof insertStaffLeaveTypeSchema>;

// Staff Leave Balances - per-staff leave balance tracking
...
// Staff Leave Requests - leave application and approval workflow
export const staffLeaveRequests = pgTable("staff_leave_requests", {
  ...
  payrollCycleId: varchar("payroll_cycle_id"),
  ...
  check("leave_request_status_valid", sql`status IN ('pending', 'approved', 'rejected', 'cancelled', 'revoked')`),
  check("half_day_type_valid", sql`half_day_type IS NULL OR half_day_type IN ('first_half', 'second_half')`),
]);

export const insertStaffLeaveRequestSchema = createInsertSchema(staffLeaveRequests).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
  approvedAt: true,
...
export type InsertStaffLeaveRequest = z.infer<typeof insertStaffLeaveRequestSchema>;

// Staff Attendance - daily attendance tracking for hourly/daily wage workers
export const staffAttendance = pgTable("staff_attendance", {
  ...
  check("attendance_status_valid", sql`status IN ('present', 'absent', 'half_day', 'on_leave', 'holiday', 'week_off')`),
]);
...
// Staff Payroll Cycles - monthly payroll run metadata
export const staffPayrollCycles = pgTable("staff_payroll_cycles", {
  ...
  check("payroll_cycle_status_valid", sql`status IN ('draft', 'processing', 'pending_approval', 'approved', 'paid', 'locked')`),
]);

export const insertStaffPayrollCycleSchema = createInsertSchema(staffPayrollCycles).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
  processedAt: true,
...
export type StaffPayrollCycle = typeof staffPayrollCycles.$inferSelect;
export type InsertStaffPayrollCycle = z.infer<typeof insertStaffPayrollCycleSchema>;

// Staff Payroll Entries - individual staff payroll calculation per cycle
export const staffPayrollEntries = pgTable("staff_payroll_entries", {
  ...
  check("payroll_payment_status_valid", sql`payment_status IN ('pending', 'processing', 'paid', 'failed')`),
]);

export const insertStaffPayrollEntrySchema = createInsertSchema(staffPayrollEntries).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
  paidAt: true,
...
export type StaffPayrollEntry = typeof staffPayrollEntries.$inferSelect;
export type InsertStaffPayrollEntry = z.infer<typeof insertStaffPayrollEntrySchema>;

// Staff Exit Records - offboarding and final settlement
export const staffExitRecords = pgTable("staff_exit_records", {
  ...
  noticePeriodServed: integer("notice_period_served").default(0),
  noticePeriodShortfall: integer("notice_period_shortfall").default(0),
  ...
  check("exit_type_valid", sql`exit_type IN ('resignation', 'termination', 'contract_end', 'retirement', 'absconding')`),
  check("settlement_status_valid", sql`settlement_status IN ('pending', 'calculated', 'approved', 'paid', 'completed')`),
  check("rehire_eligibility_valid", sql`rehire_eligibility IN ('eligible', 'not_eligible', 'with_conditions')`),
]);

export const insertStaffExitRecordSchema = createInsertSchema(staffExitRecords).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
  settledAt: true,
...
export type StaffExitRecord = typeof staffExitRecords.$inferSelect;
export type InsertStaffExitRecord = z.infer<typeof insertStaffExitRecordSchema>;

// Staff Payroll Relations
export const staffEmploymentProfilesRelations = relations(staffEmploymentProfiles, ({ one, many }) => ({
  staff: one(staff, {
    fields: [staffEmploymentProfiles.staffId],
    references: [staff.id],
  }),
  salon: one(salons, {
    fields: [staffEmploymentProfiles.salonId],
    references: [salons.id],
  }),
  salaryComponents: many(staffSalaryComponents),
  payrollEntries: many(staffPayrollEntries),
  exitRecords: many(staffExitRecords),
}));

export const staffSalaryComponentsRelations = relations(staffSalaryComponents, ({ one, many }) => ({
  employmentProfile: one(staffEmploymentProfiles, {
    fields: [staffSalaryComponents.employmentProfileId],
    references: [staffEmploymentProfiles.id],
  }),
  salon: one(salons, {
    fields: [staffSalaryComponents.salonId],
    references: [salons.id],
  }),
  createdByUser: one(users, {
    fields: [staffSalaryComponents.createdBy],
    references: [users.id],
  }),
  payrollEntries: many(staffPayrollEntries),
}));

export const staffLeaveTypesRelations = relations(staffLeaveTypes, ({ one, many }) => ({
  salon: one(salons, {
    fields: [staffLeaveTypes.salonId],
    references: [salons.id],
  }),
  leaveBalances: many(staffLeaveBalances),
  leaveRequests: many(staffLeaveRequests),
}));

export const staffLeaveBalancesRelations = relations(staffLeaveBalances, ({ one, many }) => ({
  staff: one(staff, {
    fields: [staffLeaveBalances.staffId],
    references: [staff.id],
  }),
  salon: one(salons, {
    fields: [staffLeaveBalances.salonId],
    references: [salons.id],
  }),
  leaveType: one(staffLeaveTypes, {
    fields: [staffLeaveBalances.leaveTypeId],
    references: [staffLeaveTypes.id],
  }),
  leaveRequests: many(staffLeaveRequests),
}));
...
export const staffLeaveRequestsRelations = relations(staffLeaveRequests, ({ one }) => ({
  ...
  salon: one(salons, {
    fields: [staffLeaveRequests.salonId],
    references: [salons.id],
  }),
  leaveType: one(staffLeaveTypes, {
    fields: [staffLeaveRequests.leaveTypeId],
    references: [staffLeaveTypes.id],
  }),
  leaveBalance: one(staffLeaveBalances, {
    fields: [staffLeaveRequests.leaveBalanceId],
    references: [staffLeaveBalances.id],
  }),
  approvedByUser: one(users, {
    fields: [staffLeaveRequests.approvedBy],
    references: [users.id],
  }),
}));
...
export const staffAttendanceRelations = relations(staffAttendance, ({ one }) => ({
  ...
  salon: one(salons, {
    fields: [staffAttendance.salonId],
    references: [salons.id],
  }),
  leaveRequest: one(staffLeaveRequests, {
    fields: [staffAttendance.leaveRequestId],
...
export const staffPayrollCyclesRelations = relations(staffPayrollCycles, ({ one, many }) => ({
  salon: one(salons, {
    fields: [staffPayrollCycles.salonId],
    references: [salons.id],
  }),
  processedByUser: one(users, {
    fields: [staffPayrollCycles.processedBy],
    references: [users.id],
  }),
  approvedByUser: one(users, {
    fields: [staffPayrollCycles.approvedBy],
    references: [users.id],
  }),
  payrollEntries: many(staffPayrollEntries),
}));

export const staffPayrollEntriesRelations = relations(staffPayrollEntries, ({ one }) => ({
  payrollCycle: one(staffPayrollCycles, {
    fields: [staffPayrollEntries.payrollCycleId],
    references: [staffPayrollCycles.id],
  }),
  staff: one(staff, {
    fields: [staffPayrollEntries.staffId],
    references: [staff.id],
  }),
  salon: one(salons, {
    fields: [staffPayrollEntries.salonId],
    references: [salons.id],
  }),
  employmentProfile: one(staffEmploymentProfiles, {
    fields: [staffPayrollEntries.employmentProfileId],
    references: [staffEmploymentProfiles.id],
  }),
  salaryComponent: one(staffSalaryComponents, {
    fields: [staffPayrollEntries.salaryComponentId],
    references: [staffSalaryComponents.id],
  }),
  paidByUser: one(users, {
    fields: [staffPayrollEntries.paidBy],
    references: [users.id],
  }),
}));

export const staffExitRecordsRelations = relations(staffExitRecords, ({ one }) => ({
  staff: one(staff, {
    fields: [staffExitRecords.staffId],
    references: [staff.id],
  }),
  salon: one(salons, {
    fields: [staffExitRecords.salonId],
    references: [salons.id],
  }),
  employmentProfile: one(staffEmploymentProfiles, {
    fields: [staffExitRecords.employmentProfileId],
    references: [staffEmploymentProfiles.id],
  }),
  settledByUser: one(users, {
    fields: [staffExitRecords.settledBy],
    references: [users.id],
  }),
  createdByUser: one(users, {
    fields: [staffExitRecords.createdBy],
    references: [users.id],
  }),
}));
